...
void Rent(int house_position, GAME *game_state)
{
    // pay the fees
       ...
       game_state->current_player->player_id != game_state->map[house_position].house_owner_id)
    {
        ...
        if(game_state->current_player->no_rent_time > 0){
            printf("该房子已经被%s购买\n但%s财神附身免租金\n",role_name[game_state->map[house_position].house_owner_id - 1], game_state->current_player->player_name);
...
void PayFees(GAME *game_state)
{
    ...
    if( game_state->current_player->money < fees)
    {
        map_player->money += game_state->current_player->money;
        printf("\n非常遗憾，%s已破产!\n",game_state->current_player->player_name);
